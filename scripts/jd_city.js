...
let inviteCodes = [
  "RtGKzO3xFQ6iLNeYRtY2hLPfQIJ05br8Xpd-63PslmI0E-vBwQ@RtGKzez1QwynK4KeQtJh1Ny8Ga3JErr51DrQQ0Z3P8JHsN4MQg@RtGKnov7P13LJ_nOY7RlmtM3t_HU1vWpZA1fBnsHShV1q5eB"
]
!(async () => {
  if (!cookiesArr[0]) {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://shj8are.turinglabs.net/api/v3/city/query/10/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
