...
curl命令转json： https://tool.lu/curl
*/

/*
抓包地址为"/article/complete.json"
多个请求体时用'&'号或者换行隔开" ‼️
*/

const $ = new Env("jingcai阅读")
...
if(!$.isNode()&&!YouthBody==true){
  ...
  $.msg($.name, "您未获取阅读请求，请求阅读后获取","",{'open-url':""})
 } else if (!$.isNode() && YouthBody.indexOf("&") == -1) {
  ReadArr.push(YouthBody)
 } else {
  if ($.isNode()) {
    if (process.env.JINGCAI_READ && process.env.JINGCAI_READ.indexOf('&') > -1) {
      YouthBody = process.env.JINGCAI_READ.split('&');
      console.log(`您选择的是用"&"隔开\n`)
    } else if (process.env.JINGCAI_READ && process.env.JINGCAI_READ.indexOf('\n') > -1) {
      YouthBody = process.env.JINGCAI_READ.split('\n');
      console.log(`您选择的是用换行隔开\n`)
    } else {
      YouthBody = [process.env.JINGCAI_READ]
    }
  } else if (!$.isNode() && YouthBody.indexOf("&") > -1) {
  ...
  for ( var i = indexLast ? indexLast:0; i < ReadArr.length; i++) {
    if (ReadArr[i]) {
      articlebody = ReadArr[i];
       $.index =  $.index + 1;
       $.log(`-------------------------\n\n开始jiangcai第${$.index}次阅读`);
       await $.wait(10000);
       await AutoRead();
    };
 }
   $.log("本次共阅读"+artsnum+"次资讯，共获得"+readscore+"金币\n观看"+videosnum+"次视频，获得"+videoscore+"金币(不含0金币次数)\n")
   console.log(`-------------------------\n\njiangcai共完成${$.index}次阅读，共计获得${readscore+videoscore}个金币，阅读请求全部结束`);
   $.msg($.name, `本次运行共完成${$.index}次阅读，共计获得${readscore+videoscore}个金币`)
})()
  .catch((e) => $.logErr(e))
  .finally(() => $.done())
...
function batHost(api, body) {
    return {
        ...
        headers: {
            "Host": "ant.xunsl.com",
            "os-version": "RMX2117_11_A.25",
            "device-model": "RMX2117",
            "phone-sim": "1",
            "carrier": "%E4%B8%AD%E5%9B%BD%E8%81%94%E9%80%9A",  //这是啥--中国联通
            "request_time": getTs(),   // 时间戳到秒
            "access": "4G",
            "os-api": "29",
            "app-type": "jcweather",
