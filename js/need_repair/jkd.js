...
签到一次 获取Cookie和body（jsondata开头的一大串） homeSignAccount.action
...
let UA = 'Dalvik/2.1.0 (Linux; U; Android 10; RMX2117 Build/QP1A.190711.020)'
let taskresult = "",sumnotify ="";
let calendarpic = "";
let host = 'www.xiaodouzhuan.cn'
let channel = 'oppo_a'
...
BodyArr=['jsondata={"apphotfixversion":"","appid":"xzwl","apptoken":"xzwltoken070704","appversion":"7.6.0","appversioncode":760,"channel":"ST_YINGYONGBAO01_CHANNEL","mobileinfo":"smartisan","openid":"a269c916c18d4102b53fa956a58da129","os":"android","sdktype":"bd_jssdk;bd_sdk;gdt_sdk;tt_sdk;sg_sdk;gdt_api;tt_api;zk_api;dk_api","sm_id":"DuArw0x5V7TH9XQ07ZpG6xWbIkM9XyeDjAf4UUuV93DpaypUyGjDhACqmra5JuTjvWeZSRuy80TSV9WCAHy1dQqw","token":"oRhSLRyLzE3UENpgYz%252Fb2s%252BNDPS%252FHUAx7ZV1%252BJbf5o8OJlU67mE2tPr4Ov2ehhbJ%250A"}']
cookieArr = ['JSESSIONID=16228CBE7C014F1B86CE4358A85B531B']
// BodyArr = ['jsondata={"accountType":0,"apphotfixversion":"","appid":"xzwl","apptoken":"xzwltoken070704","appversion":"7.6.0","appversioncode":760,"channel":"oppo_a","mobileinfo":"realme","openid":"ea451e77fe2346f487941f515d840ed9","os":"android","sdktype":"bd_jssdk;bd_sdk;gdt_sdk;tt_sdk;sg_sdk;gdt_api;tt_api;zk_api;dk_api","sm_id":"DuEEcehE62CrbjUZuXqxZBCDcXM0IW7PZ5aa28wwWsh9i022eULk6I+fi7P2fd4HgmLhx6nI5dcCB+Gtri6lLFtw","token":"S0xkcFFCzF3l5AMBwTg1Oxo0if5ne9Tm282HKNbBVMPQMrC8QhTaGCIU%252FiRC8mWX%250A"}']
// cookieArr = ['JSESSIONID=B9854201587033887E3D63245D1F72C6']
...
!(async() => {
  if (!BodyArr[0]) {
    console.log($.name, '【提示】请把聚看点Cookie export到全局变量中，请以&或者换行隔开')
    return;
  }
  ...
  for (let i = 0; i < BodyArr.length; i++) {
    if (BodyArr[i]) {
      ...
      bodyval = BodyArr[i]
      bodyorigin = JSON.parse(BodyArr[i].slice(9,))
      console.log("解析token成功",bodyorigin['token'])
      // 取用户openid
      ID =  bodyorigin['openid']
      // 取apptoken
      apptoken = bodyorigin['apptoken']
      // 取appversioncode
      appVersion = bodyorigin['appversion']
      // 用哪个cookieval
...
function getsign() {
  return new Promise((resolve, reject) =>{
   let signurl =  {
      ...
      headers: {Cookie:cookieval,'User-Agent':UA,Host:'www.xiaodouzhuan.cn'},
      body: bodyval
      }
...
function signShare() {
  return new Promise((resolve, reject) =>{
   let profiturl =  {
      ...
      headers: {Cookie:cookieval,'User-Agent':UA,Host:'www.xiaodouzhuan.cn'}, body: bodyval
      }
   $.post(profiturl, async(error, resp, data) => {
     //$.log(data+"\n")
...
function LuckBox(x) {
  return new Promise((resolve, reject) =>{
   let Boxurl =  {
      ...
      headers: {Cookie:cookieval,'User-Agent':UA,Host:'www.xiaodouzhuan.cn'}, body: `num=${x}`
      }
...
function realname() {
  return new Promise((resolve, reject) =>{
   let realurl =  {
      ...
      headers: {Cookie:cookieval,Host:'www.xiaodouzhuan.cn'}
      }
...
function Withdraw() {
  return new Promise((resolve, reject) =>{
   let drawurl =  {
      ...
      headers: {Host:'www.xiaodouzhuan.cn',Cookie:cookieval,'User-Agent':UA,'Referer': 'https://www.xiaodouzhuan.cn/jkd/weixin20/userWithdraw/userWithdraw.action'}, body: `type=wx&sum=${sumcash}&mobile=&pid=0&accountid=&productcode=`
      }
...
function artList(readbodyVal) {
  return new Promise((resolve, reject) =>{
   let infourl =  {
      ...
      headers: {Cookie:cookieval,'User-Agent':UA,Host:host},
      body: readbodyVal
      }
   $.post(infourl, async(error, resp, data) => {
...
function readTask(artid,arttype) {
  return new Promise((resolve, reject) =>{
   let rewurl =  {
      url: `https://www.xiaodouzhuan.cn/jkd/newmobile/articleDetail.action`,
      headers: {Cookie:cookieval,'User-Agent':UA},
      body: `jsondata={"appid":"xzwl","channel":${channel},"relate":1,"artid":"${artid}","os":"IOS","openid":"${ID}","apptoken":"${apptoken}","appversion":"${appVersion}"}`
      }
...
function invite() {
   let rewurl =  {
      url: `https://www.xiaodouzhuan.cn/jkd/weixin20/member/bindTeacher.action?teacherCode=1912011`,
      headers: {Cookie:cookieval, "user-agent":"Mozilla/5.0 (Linux; Android 10; RMX2117 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/85.0.4183.127 Mobile Safari/537.36"},
      body: bodyval
      }
